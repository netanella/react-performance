...
Wrapping a component with React.memo will make your component not re-render
simply because its parent re-rendered, which could improve the app performance.

Sometimes it can cause the opposite and slow things down so needs to be used 
intentionally.

Kent likes to re-assign the component like so:
```javascript
Menu = React.memo(Menu)
```

Custom comparator - We can pass React.memo a second argument, a function 
that takes the previous and next props and re-renders the component if 
"false" is returned.

Implementing this function can be tricky and sometimes we can avoid the 
unwanted re-renders by simply making sure to move calculations higher in 
the tree and passing only primitive values.
