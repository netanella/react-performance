...
Load only what's needed, when it's needed, with React.lazy(()=> import(path)

When using this technique - the loaded component must be wrapped with a Suspense
with a fallback.

"coverage" feature in the dev tools - helps to analyze your app to determine the
need/benefit of code splitting for a certain feature/page/interaction.

Test the app using "Slow 3G" setting in the Network tab.

Kick off a dynamic import in event handlers that indicate the user want to
interact with that code. It doesn't matter how many times we call the import
function because it gets cached by the browser.

If there is no reason to wait for user interaction - we can start loading it
right away when we're done with the other stuff using webpack "magic comments":

```javascript
import(/* webpackPrefetch: true */ './some-module.js')
```

We can see those prefetched modules in the Elements tab, <head> links.

Useful tool: in react Component devtool, we can "suspend" a certain component
