...
"Performance" tab - when recording the rerender, look for a particular function
that takes a long time (has a long line) or has many sub-tasks (short lines)
under it.

We use useMemo to prevent unwanted re-renders that can be caused by a parent
rerender or an update to an unrelated state.

To speed up the function run itself, we can take advantage of web workers to
split the task to a separate thread, so our main thread is free to do other
stuff.
